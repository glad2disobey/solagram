...
pub struct CreateProfile<'info> {
  ...
  #[account(
    ...
    space = utils::pubkeys::PubkeyList::space_for(0, constants::MAX_PROFILE_SESSION_LIST_LENGTH).unwrap(),
    seeds = [
  ...
  #[account(
    ...
    space = utils::pubkeys::PubkeyList::space_for(0, constants::MAX_PROFILE_PENDING_SESSION_LIST_LENGTH).unwrap(),
    seeds = [
...
pub fn create_profile(
  ...
  profile_session_list_state.pubkeys = Vec::new();
  profile_session_list_state.bump = ctx.bumps.profile_session_list_state;

  let profile_pending_session_list_state = &mut ctx.accounts.profile_pending_session_list_state;
  profile_pending_session_list_state.pubkeys = Vec::new();
  profile_pending_session_list_state.bump = ctx.bumps.profile_pending_session_list_state;
...
pub struct CreateTokenAccount<'info> {
  ...
  #[account(
    ...
    constraint = platform_token_state.mint_address.key() == mint.key()
  ...
  #[account(
    ...
    constraint = token_plugin_list_state.pubkeys.contains(&token_plugin)
...
pub fn create_token_account(
    ...
    && platform_token_treasury_state.amount >= platform_token_state.airdrop_amount
  {
    ...
    transfer_checked(
      ...
      platform_token_state.airdrop_amount, mint.decimals,
    )?;
  }
